...
class AgentTestSuite:
    ...
    def test_legal_compliance_agent(self):
        ...
        if data_rights_file.exists():
            ...
            has_validation = ('validate_excerpt_length' in content or 
                            'can_export' in content or 
                            'ExportValidator' in content or
                            'finalize_export' in content)
            
            if has_validation:
                self.log_test('legal-compliance', test_name, 'PASS',
                             "data_rights.py contains export validation logic")
            else:
                self.log_test('legal-compliance', test_name, 'FAIL',
                             "data_rights.py missing export validation functions")
        else:
            self.log_test('legal-compliance', test_name, 'FAIL',
                         "data_rights.py file not found")
        ...
        missing_docs = [doc for doc in copyright_docs 
        ...
        if models_file.exists():
            ...
            self.log_test('legal-compliance', test_name, 'FAIL',
                         "models_data_rights.py not found")
    ...
    def test_bwc_forensics_agent(self):
        ...
        if analyzer_file.exists():
            ...
            if has_analysis:
                self.log_test('bwc-forensics', test_name, 'PASS',
                             f"Analyzer has {len(content)} chars of analysis code")
            else:
                self.log_test('bwc-forensics', test_name, 'FAIL',
                             "Analyzer file too small - may be incomplete")
        else:
            self.log_test('bwc-forensics', test_name, 'FAIL',
                         "bwc_forensic_analyzer.py not found")
        ...
        if web_app_file.exists() and html_file.exists():
            self.log_test('bwc-forensics', test_name, 'PASS',
                         "Both backend and frontend files present")
        else:
            missing = []
        ...
        if guide_file.exists():
            ...
            self.log_test('bwc-forensics', test_name, 'FAIL',
                         "BWC-ANALYSIS-GUIDE.md not found")
    ...
    def test_flask_backend_agent(self):
        ...
        if app_file.exists():
            ...
            if has_flask and has_app:
                self.log_test('flask-backend', test_name, 'PASS',
                             "app.py contains valid Flask application")
            else:
                self.log_test('flask-backend', test_name, 'FAIL',
        ...
        if auth_file.exists() and models_file.exists():
            ...
            if has_login:
                self.log_test('flask-backend', test_name, 'PASS',
                             "auth_routes.py and models_auth.py present")
            else:
                self.log_test('flask-backend', test_name, 'FAIL',
                             "auth_routes.py missing login functionality")
        else:
            self.log_test('flask-backend', test_name, 'FAIL',
                         "Missing auth_routes.py or models_auth.py")
    ...
    def test_frontend_dev_agent(self):
        ...
        missing_pages = [page for page in html_pages 
                        if not (self.repo_root / page).exists()]
        
        if not missing_pages:
            self.log_test('frontend-dev', test_name, 'PASS',
                         f"All {len(html_pages)} core HTML pages present")
        else:
            self.log_test('frontend-dev', test_name, 'FAIL',
        ...
        if css_dir.exists() and style_file.exists():
            ...
            if component_css:
                ...
                self.log_test('frontend-dev', test_name, 'FAIL',
                             "Missing component-based CSS architecture")
        else:
            self.log_test('frontend-dev', test_name, 'FAIL',
                         "assets/css/ directory structure not found")
        ...
        existing_docs = [doc for doc in frontend_docs 
        ...
        if existing_docs:
            ...
            self.log_test('frontend-dev', test_name, 'FAIL',
                         "No frontend documentation found")
    ...
    def test_database_architect_agent(self):
        ...
        existing_models = [model for model in model_files 
                          if (self.repo_root / model).exists()]
        
        if len(existing_models) == len(model_files):
            self.log_test('database-architect', test_name, 'PASS',
                         "All database model files present")
        else:
            missing = set(model_files) - set(existing_models)
            self.log_test('database-architect', test_name, 'FAIL',
                         f"Missing: {', '.join(missing)}")
        ...
        existing_migrations = [script for script in migration_scripts 
        ...
        existing_validation = [script for script in validation_scripts 
                              if (self.repo_root / script).exists()]
        ...
        if existing_validation:
            ...
            self.log_test('database-architect', test_name, 'FAIL',
                         "No database validation scripts found")
    ...
    def test_security_devops_agent(self):
        ...
        existing_docs = [doc for doc in security_docs 
                        if (self.repo_root / doc).exists()]
        
        if len(existing_docs) == len(security_docs):
            self.log_test('security-devops', test_name, 'PASS',
                         "All security documentation present")
        else:
            missing = set(security_docs) - set(existing_docs)
            self.log_test('security-devops', test_name, 'FAIL',
                         f"Missing: {', '.join(missing)}")
        ...
        existing_deps = [dep for dep in dep_files 
    ...
    def test_documentation_agent(self):
        ...
        existing_core = [doc for doc in core_docs 
                        if (self.repo_root / doc).exists()]
        
        if len(existing_core) == len(core_docs):
            self.log_test('documentation', test_name, 'PASS',
                         "All core documentation files present")
        else:
            missing = set(core_docs) - set(existing_core)
            self.log_test('documentation', test_name, 'FAIL',
                         f"Missing: {', '.join(missing)}")
        ...
        existing_guides = [guide for guide in user_guides 
                          if (self.repo_root / guide).exists()]
        ...
        existing_agent_docs = [doc for doc in agent_docs 
                              if (self.repo_root / doc).exists()]
