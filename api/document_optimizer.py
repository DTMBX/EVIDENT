...
def generate_filing_set_report(results: List[OptimizationResult], jurisdiction: str) -> str:
    ...
    total_issues = sum(len(r.consistency_issues) + len(r.evidence_gaps) + len(r.procedural_issues) for r in results)
    ...
    report_lines.append("-" * 80)
    report_lines.append("This optimization report is provided as a drafting and organizational assistant.")
    report_lines.append("It does NOT constitute legal advice. All documents must be reviewed by a licensed")
