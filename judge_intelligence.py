...
class JudgeIntelligence:
    ...
    def _get_school_prestige(self, school_name: str) -> str:
        ...
        # Top 14 law schools
        t14 = ['Yale', 'Stanford', 'Harvard', 'Chicago', 'Columbia', 'NYU', 
               'Penn', 'Virginia', 'Michigan', 'Duke', 'Northwestern', 
               'Cornell', 'Berkeley', 'Georgetown']
    ...
    def get_notable_opinions(self, judge_id: int, limit: int = 10) -> List[Dict]:
        ...
        return [{
            'note': 'Notable opinions require opinion corpus with citation counts'
        }]
...
def analyze_judge_for_case(judge_name: str, case_type: str) -> Dict:
    ...
    for edu in education:
        if edu['school_prestige'] == 'T14 (Elite)':
            recommendations.append(f"✓ Judge attended {edu['school']} - expect sophisticated legal analysis")
    
    return {
        'judge': profile['basic_info']['name'],
        'ideology': ideology,
        'education_highlights': [edu for edu in education if edu['school_prestige'] == 'T14 (Elite)'],
