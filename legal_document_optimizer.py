...
class LegalDocumentOptimizer:
    ...
    def optimize_document(
        ...
        system_prompt = self._build_optimizer_prompt(document.doc_type, jurisdiction, optimization_goals)
...
"""
    ...
    def _analyze_optimization_changes(
        self,
        original: str,
        optimized: str
    ) -> Dict:
    ...
    def _parse_optimized_output(self, optimized: str) -> Dict:
        ...
        if strategic_match:
            ...
            sections['strategic_improvements'] = [
                line.strip() for line in strategic_text.split('\n')
                if line.strip() and not line.strip().startswith('**')
            ]
