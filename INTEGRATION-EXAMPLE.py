...
event_bus.subscribe('evidence.processing_failed', lambda e: logger.error(f"Processing failed: {e.data}"))
...
@api_endpoint(
    ...
    require_auth=True,
    min_tier='enterprise',
    validation_schema={'required': ['case_number']}
)
...
def admin_performance_metrics():
    ...
    return render_template(
        ...
        metrics=stats,
        service_registry=service_registry.list_services()
    )
