...
class FreeTrierUser(BarberXUser):
    ...
    def attempt_one_time_upload(self):
        ...
        self.client.post("/api/upload/free", 
                        json={"file_type": "pdf", "size_mb": 5})
...
class StarterTierUser(BarberXUser):
    ...
    def on_start(self):
        ...
        response = self.client.post("/auth/login", json={
            "email": f"starter_{random.randint(1, 100)}@test.com",
    ...
    def upload_video(self):
        """Upload BWC video."""
        self.client.post("/api/upload/video", 
                        json={"file_size_mb": 256, "duration_minutes": 15})
    
    @task(6)
    @tag('upload', 'starter')
    def upload_pdf(self):
        """Upload PDF document."""
        self.client.post("/api/upload/pdf", 
                        json={"file_size_mb": 5, "pages": 25})
    ...
    def export_report(self):
        ...
        case_id = random.randint(1, 100)
        self.client.post(f"/api/export/report", json={
            "case_id": case_id,
            "format": "pdf"
        })
...
class ProfessionalTierUser(BarberXUser):
    ...
    def on_start(self):
        ...
        response = self.client.post("/auth/login", json={
            "email": f"pro_{random.randint(1, 50)}@test.com",
            "password": "testpass123"
        })
    ...
    def batch_upload_videos(self):
        ...
        self.client.post("/api/upload/batch", json={
            "files": [
    ...
    def run_advanced_analysis(self):
        ...
        self.client.post(f"/api/analysis/forensic", json={
            "case_id": case_id,
    ...
    def generate_timeline(self):
        ...
        case_id = random.randint(1, 200)
        self.client.post(f"/api/timeline/generate", json={
            "case_id": case_id
        })
    ...
    def legal_research_query(self):
        ...
        query = random.choice([
            "Fourth Amendment", "Miranda Rights", "Qualified Immunity",
            "Brady Violation", "Excessive Force"
        ])
        self.client.post("/api/legal-library/search", json={
            "query": query,
...
class PremiumTierUser(BarberXUser):
    ...
    def api_batch_process(self):
        ...
        self.client.post("/api/v1/batch/process", 
    ...
    def api_forensic_analysis(self):
        ...
        self.client.post("/api/v1/forensic/analyze",
                        headers={"X-API-Key": self.api_key},
                        json={
    ...
    def multi_video_sync(self):
        ...
        self.client.post("/api/multi-bwc/sync", json={
            "video_ids": [random.randint(1, 1000) for _ in range(3)]
        })
    ...
    def export_advanced_report(self):
        ...
        self.client.post("/api/export/advanced", json={
            "case_id": random.randint(1, 500),
            "format": "docx",
...
class EnterpriseTierUser(BarberXUser):
    ...
    def api_bulk_upload(self):
        ...
        self.client.post("/api/v1/bulk/upload",
                        headers={"X-API-Key": self.api_key},
                        json={
                            "files": [
                                {"type": "video", "size_gb": random.uniform(1, 10)}
                                for _ in range(20)
                            ]
                        })
    ...
    def generate_white_label_report(self):
        ...
        self.client.post("/api/v1/export/white-label",
                        headers={"X-API-Key": self.api_key},
                        json={
