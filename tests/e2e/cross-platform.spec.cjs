...
test.describe("API - Architecture Boundaries", () => {
  ...
  test("API returns JSON content type for API endpoints", async ({
    request,
  }) => {
...
test.describe("Mobile API - Offline Support", () => {
  ...
  test("API error responses include proper status codes", async ({
    request,
  }) => {
...
test.describe("Architecture - Shared Layer Integration", () => {
  test("API endpoints use consistent naming convention", async ({
    request,
  }) => {
    const endpoints = [
      "/api/user/profile",
