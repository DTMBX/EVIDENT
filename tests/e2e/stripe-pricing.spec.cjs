...
test.describe("Stripe Pricing Table - Integration", () => {
  test("pricing-stripe-embed.html loads Stripe script with crossorigin", async ({
    page,
  }) => {
    await page.goto("/pricing-stripe-embed.html");

    // Check that Stripe script is loaded with crossorigin attribute
    const stripeScript = page.locator(
      'script[src*="js.stripe.com/v3/pricing-table.js"]',
    );
  ...
  test("pricing.html loads Stripe script with crossorigin", async ({
    page,
  }) => {
  ...
  test("landing page loads Stripe script with crossorigin", async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("Stripe Pricing Table - COEP Policy", () => {
  ...
  test("Stripe script loads without COEP blocking", async ({ page }) => {
    ...
    page.on("pageerror", (err) => {
      if (
        err.message.includes("ERR_BLOCKED_BY_RESPONSE") ||
...
test.describe("Pricing Tiers - Visual Elements", () => {
  ...
  test("pricing page has proper styling", async ({ page }) => {
    ...
    const pricingWrapper = page.locator(
      ".stripe-pricing-wrapper, .pricing-container, .pricing-section",
    );
  ...
  test("pricing header displays correctly", async ({ page }) => {
    ...
    const header = page
      .locator("h1, h2")
      .filter({ hasText: /pricing|choose|plan/i });
...
test.describe("Pricing - Navigation", () => {
  test("pricing page has link to home", async ({ page }) => {
    ...
    const homeLink = page.locator(
      'a[href="/"], a[href="index.html"], a[href*="home"]',
    );
...
test.describe("Stripe Pricing - Console Errors", () => {
  ...
  test("no COEP blocking errors in console", async ({ page }) => {
    ...
    const hasCoepError = consoleErrors.some(
      (err) =>
