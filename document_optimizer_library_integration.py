...
class DocumentOptimizerLibraryIntegration:
    ...
    def suggest_citations_for_document(self, document_text: str, doc_type: str = None) -> List[Dict]:
        """
        ...
        for issue in legal_issues:
            ...
            cases = self.library.search_library(
                query=issue,
                doc_type='case',
                limit=3
            )
    ...
    def enhance_optimization_prompt(self, base_prompt: str, document_text: str) -> str:
        ...
        citation_context += "Only cite cases if they genuinely strengthen the document. Do not force citations.\n"
    ...
    def _extract_legal_issues(self, text: str) -> List[str]:
        ...
        issue_indicators = [
            'excessive force', 'unlawful search', 'unlawful seizure',
            'miranda violation', 'due process violation', 'equal protection',
            'qualified immunity', 'municipal liability', 'monell claim',
            'brady violation', 'prosecutorial misconduct', 'ineffective assistance',
            'warrantless search', 'probable cause', 'reasonable suspicion',
